                                                        ...
                                                        <div class="text-sm font-light ml-2 text-gray-500">
                                                                ...
                                                                @foreach($user->trackLists as $tracks)
                                                                    @php
                                                                        ...
                                                                        if($tracks->status === 'Отправлено в Казахстан'){ $date = \Illuminate\Support\Carbon::parse($tracks->to_china)->format('Y-m-d'); }
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        weight = $("#weight").val();
        $("#tengeSum").val(((weight * 4.3) * {{$config->rate}}).toFixed())
        $("#tengeSumPer").val(((((weight * 4.3) * {{$config->rate}})) + ((((weight * 4.3) * {{$config->rate}})))/100).toFixed())
    });
